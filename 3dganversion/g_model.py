...
class MR2CT(object):
    def __init__(self, sess, batch_size=10, depth_MR=32, height_MR=32,
                 width_MR=32, depth_CT=16, height_CT=16,
                 width_CT=16, l_num=2, wd=0.0005, checkpoint_dir=None, path_patients_h5=None, learning_rate=2e-8,lr_step=30000):
        """
        Args:
    ...
    def generator(self,inputMR,batch_size_tf):
        ...
        conv1_a = conv_op_3d_bn(inputMR, name="conv1_a", kh=9, kw=9, kz=9,  n_out=32, dh=1, dw=1, dz=1, wd=self.wd, padding='VALID',train_phase=self.train_phase)#30
        conv2_a = conv_op_3d_bn(conv1_a, name="conv2_a", kh=3, kw=3, kz=3,  n_out=32, dh=1, dw=1, dz=1, wd=self.wd, padding='SAME',train_phase=self.train_phase)
        conv3_a = conv_op_3d_bn(conv2_a, name="conv3_a", kh=3, kw=3, kz=3,  n_out=32, dh=1, dw=1, dz=1, wd=self.wd, padding='SAME',train_phase=self.train_phase)#28
        conv4_a = conv_op_3d_bn(conv3_a, name="conv4_a", kh=3, kw=3, kz=3,  n_out=32, dh=1, dw=1, dz=1, wd=self.wd, padding='SAME',train_phase=self.train_phase)#28
        conv5_a = conv_op_3d_bn(conv4_a, name="conv5_a", kh=9, kw=9, kz=9,  n_out=64, dh=1, dw=1, dz=1, wd=self.wd, padding='VALID',train_phase=self.train_phase)
        
        conv6_a = conv_op_3d_bn(conv5_a, name="conv6_a", kh=3, kw=3, kz=3,  n_out=64, dh=1, dw=1, dz=1, wd=self.wd, padding='SAME',train_phase=self.train_phase)#26
        conv7_a = conv_op_3d_bn(conv6_a, name="conv7_a", kh=3, kw=3, kz=3,  n_out=64, dh=1, dw=1, dz=1, wd=self.wd, padding='SAME',train_phase=self.train_phase)
        conv8_a = conv_op_3d_bn(conv7_a, name="conv8_a", kh=3, kw=3, kz=3,  n_out=32, dh=1, dw=1, dz=1, wd=self.wd, padding='SAME',train_phase=self.train_phase)
        ...
        conv9_a = conv_op_3d_norelu(conv8_a, name="conv9_a", kh=3, kw=3, kz=3,  n_out=1, dh=1, dw=1, dz=1, wd=self.wd, padding='SAME')#24 I modified it here,dong
        #self.MR_16_downsampled=conv7_a#JUST FOR TEST
        return conv9_a,conv9_a


    def discriminator(self, inputMR, reuse=False):
    	if reuse:
	        tf.get_variable_scope().reuse_variables()
        h0=conv_op_3d(inputMR, name="conv_dis_1_a", kh=5, kw=5, kz=5,  n_out=32, dh=1, dw=1, dz=1, wd=self.wd, padding='SAME')
        h1 = conv_op_3d_bn(h0, name="conv2_dis_a", kh=3, kw=3, kz=3,  n_out=64, dh=1, dw=1, dz=1, wd=self.wd, padding='SAME',train_phase=self.train_phase)
        h2 = conv_op_3d_bn(h1, name="conv3_dis_a", kh=3, kw=3, kz=3,  n_out=64, dh=1, dw=1, dz=1, wd=self.wd, padding='SAME',train_phase=self.train_phase)#28
        h3 = conv_op_3d_bn(h2, name="conv4_dis_a", kh=3, kw=3, kz=3,  n_out=32, dh=1, dw=1, dz=1, wd=self.wd, padding='SAME',train_phase=self.train_phase)
    ...
    def train(self, config):
        ...
        stack_losses=collections.deque(5*[0], 5)#average of last five iterations
        ...
        for it in range(start,config.iterations):
            ...
            if it%config.test_every==0 and it!=0:#==0:#test one subject                
                ...
                print mrnp.dtype
                print ctnp.dtype
                ct_estimated=self.test_1_subject(mrnp,ctnp,[32,32,32],[16,16,16],[2,5,5])
